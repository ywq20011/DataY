package com.gaoding.datay.core.util.container;
...
import com.gaoding.datay.common.plugin.AbstractPlugin;
...
public class LoadUtil {
    ...
    private static Configuration getPluginConf(PluginType pluginType, String pluginName) {
        String generatePluginKey = generatePluginKey( pluginName);
        Configuration pluginConf = pluginRegisterCenter.getConfiguration(generatePluginKey);
    ...
    private static synchronized Class<? extends AbstractPlugin> loadPluginClass(PluginType pluginType,
       ...
       // JarLoader jarLoader = LoadUtil.getJarLoader(pluginType, pluginName);
        try {
            return (Class<? extends AbstractPlugin>)Class.forName(pluginConf.getString("class") + "$" + pluginRunType.value());
    ...
    public static synchronized JarLoader getJarLoader(PluginType pluginType, String pluginName) {
        ...
        JarLoader jarLoader = jarLoaderCenter.get(generatePluginKey( pluginName));
        if (null == jarLoader) {
            ...
            jarLoaderCenter.put(generatePluginKey( pluginName), jarLoader);
        }
