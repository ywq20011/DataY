package com.gaoding.datay.core.job;
...
import com.gaoding.datay.common.statistics.PerfTrace;
import com.gaoding.datay.common.statistics.VMInfo;
import com.gaoding.datay.common.util.Configuration;
import com.gaoding.datay.common.util.StrUtil;
import com.gaoding.datay.core.AbstractContainer;
import com.gaoding.datay.core.Engine;
import com.gaoding.datay.core.container.util.HookInvoker;
import com.gaoding.datay.core.container.util.JobAssignUtil;
import com.gaoding.datay.core.job.scheduler.AbstractScheduler;
import com.gaoding.datay.core.job.scheduler.processinner.StandAloneScheduler;
import com.gaoding.datay.core.statistics.communication.Communication;
import com.gaoding.datay.core.statistics.communication.CommunicationTool;
import com.gaoding.datay.core.statistics.container.communicator.AbstractContainerCommunicator;
import com.gaoding.datay.core.statistics.container.communicator.job.StandAloneJobContainerCommunicator;
import com.gaoding.datay.core.statistics.plugin.DefaultJobPluginCollector;
import com.gaoding.datay.core.util.ErrorRecordChecker;
import com.gaoding.datay.core.util.FrameworkErrorCode;
import com.gaoding.datay.core.util.container.ClassLoaderSwapper;
import com.gaoding.datay.core.util.container.CoreConstant;
import com.gaoding.datay.core.util.container.LoadUtil;
import com.gaoding.datay.dataxservice.face.domain.enums.ExecuteMode;
...
public class JobContainer extends AbstractContainer {
        ...
        public void start() {
                ...
                try {
                        ...
                        Communication reportCommunication = CommunicationTool.getReportCommunication(communication,
                                        tempComm, this.totalStage);
        ...
        private void preHandle() {
                String handlerPluginTypeStr = this.configuration
                                .getString(CoreConstant.DATAX_JOB_PREHANDLER_PLUGINTYPE);
             ...
             //   classLoaderSwapper.setCurrentThreadClassLoader(
              //                  LoadUtil.getJarLoader(handlerPluginType, handlerPluginName));
        ...
        private void postHandle() {
                String handlerPluginTypeStr = this.configuration
                                .getString(CoreConstant.DATAX_JOB_POSTHANDLER_PLUGINTYPE);
        ...
        private int split() {
                 ...
                 */
                List<Configuration> contentConfig = mergeReaderAndWriterTaskConfigs(readerTaskConfigs,
                                writerTaskConfigs, transformerList);
        ...
        private void adjustChannelNumber() {
                ...
                if (isByteLimit) {
                        long globalLimitedByteSpeed = this.configuration
                                        .getInt(CoreConstant.DATAX_JOB_SETTING_SPEED_BYTE, 10 * 1024 * 1024);
                ...
                if (isRecordLimit) {
                        long globalLimitedRecordSpeed = this.configuration
                                        .getInt(CoreConstant.DATAX_JOB_SETTING_SPEED_RECORD, 100000);
                ...
                boolean isChannelLimit = (this.configuration.getInt(CoreConstant.DATAX_JOB_SETTING_SPEED_CHANNEL,
                                0) > 0);
                if (isChannelLimit) {
        ...
        private void schedule() {
                 ...
                 */
                int channelsPerTaskGroup = this.configuration
                                .getInt(CoreConstant.DATAX_CORE_CONTAINER_TASKGROUP_CHANNEL, 5);
        ...
        public Map<String,Object> getStatistics(){
                ...
                map.put("totalCosts", totalCosts);
                map.put("byteSpeedPerSecond", byteSpeedPerSecond);
                map.put("recordSpeedPerSecond", recordSpeedPerSecond);
                map.put("totalReadRecords", CommunicationTool.getTotalReadRecords(communication));
                map.put("totalErrorRecords", CommunicationTool.getTotalErrorRecords(communication));
        ...
        public void logStatistics() {
                ...
                LOG.info(String.format(
                                ...
                                "任务总计耗时", String.valueOf(totalCosts) + "s", "任务平均流量",
                                StrUtil.stringify(byteSpeedPerSecond) + "/s", "记录写入速度",
        ...
        private void invokeHooks() {
                ...
                HookInvoker invoker = new HookInvoker(CoreConstant.DATAX_HOME + "/hook", configuration,
                                comm.getCounter());
                invoker.invokeAll();
        }
}
